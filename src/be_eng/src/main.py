    ...
    if request.method == 'POST':
        ...
        if mess['status']:
            ...
            user = db.get_document(
                'user', {'name': name, 'pass': passw}).get("data")
            if user:
                token = str(uuid4())
                message = db.update_document(
...
def topic():
    ...
    if request.method == "POST":
        ...
        if (response.status_code != 404):
            ...
            for mean in meanings:
                print(mean.get("example"))
                newDict = {
                    "type": mean.get("partOfSpeech"),
                    "synonyms": mean.get("synonyms"),
                    "antonyms": mean.get("antonyms"),
                    "definition": mean.get("definitions")
                }
    ...
    if request.method == "PUT":
        ...
        if (len(str(data.get('key'))) < 1 or len(str(data.get('value'))) < 1):
            ...
            info_response = db.update_document(
                table, {'_id': ObjectId(table1)}, info)
...
def showexample():
    ...
    if request.method == "GET":
        ...
        for i in k:
            ...
            if (len_col < 13):
                list_ext_id = db.get_document(
                    i, {'level': {'$gt': 5}}).get("data")
            else:
                list_ext_id = db.get_records_top(
                    i, "level", 1, (len_col - 12)).get("data")
            dict = {}
            dict['table'] = i
...
def extension():
    if request.method == "GET":
        ...
        info_response = {'total': total, 'expert': expert,
                         'vague': vague, 'forget': forget, 'data': list_ext_id}
    if request.method == "POST":
        data = json.loads(request.data)
        k = db.show_allcolection()
    ...
    if request.method == 'GET':
        ...
        for i in k:
            if (i != 'user') and (i != 'note'):
                list_ext_id = db.get_limit(
                    i, {'level': {'$gt': int(a)}}, 44).get("data")
                if list_ext_id:
...
def topics():
    if request.method == 'GET':
        ...
        for it in newlist:
            ...
            fields = ['key', 'audio']
            result = list(
                map(lambda d: {field: d[field] for field in fields}, dataIntable))
            it['word'] = result
            print(it['word'])

          ...
          <el-col :xs="24" :sm="24" :md="24">
            <el-card shadow="never">
              <div slot="header" class="clearfix" style="position: relative;">
                <span style="font-weight: bold; font-size: 20px">Test IPA</span>
...
<!--                      <h2>{{item.key}}</h2>-->
                      <p>{{count}}</p>
                      <el-button @click="playSound(item.spell)" type="warning" icon="el-icon-phone-outline"></el-button>
                        <div v-if="item.type === '1'">Nguyên âm đơn</div>
                        <div v-if="item.type === '2'">Nguyên âm đôi</div>
                        <div v-if="item.type === '3'">Phụ âm</div>
                        <div v-if="item.type === '4'">Phụ âm</div>
                    </center>
                  </el-col>
                  <el-col :span="16">
                    <div v-if="check">
                ...
                <el-row style="padding-top: 40px">
                  <center>
                    <el-button @click="memorize()" type="danger">Kết thúc</el-button>
                  </center>
                </el-row>
                <div>
                  <el-table
...
export default {
  ...
  methods: {
    ...
    checkAnser() {
      ...
      this.answer[y] = this.item.ipa
      for (const i in this.answer) {
        console.log('+++===' + i)
        if (this.answer[i] === '') {
          var x1 = Math.floor((Math.random() * this.words.length) + 0)
          if (this.answer.indexOf(this.words[x1].ipa) < 0) {
            this.answer[i] = this.words[x1].ipa
          } else {
            for (let i1 = 0; i1 < this.words.length; i1++) {
              if (this.answer.indexOf(this.words[i1].ipa) < 0) {
                this.answer[i] = this.words[i1].ipa
              }
            }
          }
        }
    ...
    exampale() {
      ...
      var audio = new Audio(this.item.spell)
      audio.play()
    },
    startTimer: function() {
